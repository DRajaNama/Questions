...
<div class="row" style="padding:5px 35px 10px 50px ">
		<div class="col-lg-4">
			...
			<div class="row">
				...
				<h3>Questions : <?php echo $states['total']; ?></h3>
				</div>
			</div>
			</div>
		</div>
...
<div class="row" style="margin:5px 35px 0px 35px; border: 2px solid #e3e3e3;padding: 15px !important;border-top: 5px solid #fd3768;">
		<div class="col-lg-4">
			<h5 style="color:#fd3768"><strong>Search Question's with Filters?</strong></h5>
		...
		<div class="col-lg-2" style="padding-top:20px">
			<div class="input-group">
			  <select  class="form-control"  ng-model="filter" ng-change="getFilterData()" width="100%" style="height:40px !important">
				...
				<option value="viewers">BY Viewers</option>
			  </select>
			</div>
		</div>
		<div class="col-lg-6" style="padding-top:20px">
			<!--div class="input-group"-->
			  <input type="text" class="form-control" placeholder="Search for..."  ng-change="getFilterData()" ng-model="searchKey"  aria-label="Search for...">
...
<div class="panel panel-default">
  ...
  <h1 style="color:#2e3458">Questions List</h1>
  <table class="table table-responsive">
    ...
    <tbody>
      ...
      <tr ng-repeat="x in all_question">
		...
		<td>{{x.question}}</td>
		<td>{{x.category}}</td>
		<td>{{x.total_no_answer}}</td>
		<td>{{x.viewers}}</td>
		<td>
			<span ng-repeat="tag in x.taags">
		...
		<td>
		<label class="switch">
		  <input type="checkbox"  ng-checked="question_status(x)" ng-click="myfunc(x)" ng-model="status" >
		...
		<a  href="<?php echo base_url(); ?>question/{{x.id}}" target="blank" class="btn btn-success btn-small"><i class="fa fa-external-link"></i></a>
		</td>
      </tr>
